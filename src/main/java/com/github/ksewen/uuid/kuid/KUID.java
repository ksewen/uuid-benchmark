...
/**
 * A class that represents an immutable universally unique identifier edit by Ksewen (KUID). A KUID
...
public final class KUID implements Serializable, Comparable<KUID> {
  ...
  /**
   * Constructs a new {@code KUID} using the specified data. {@code mostSigBits} is used for the
   * most significant 64 bits of the {@code KUID} and {@code leastSigBits} becomes the least
   * significant 64 bits of the {@code KUID}.
   *
   ...
   *
   * <p>The {@code KUID} is generated using a cryptographically strong pseudo random number
   * generator.
   *
   * @param random
  ...
  /**
   * The version number associated with this {@code KUID}. The version number describes how this
   * {@code KUID} was generated.
   *
   * <p>The version number has the following meaning:
   *
   * <ul>
   ...
   * argument is not {@code null}, is a {@code KUID} object, has the same variant, and contains the
   * same value, bit for bit, as this {@code KUID}.
   *
   ...
   *
   * <p>The first of two KUIDs is greater than the second if the most significant field in which the
   * KUIDs differ is greater for the first KUID.
   *
   * @param val {@code KUID} to which this {@code KUID} is to be compared
   * @return -1, 0 or 1 as this {@code KUID} is less than, equal to, or greater than {@code val}
   */
  @Override
